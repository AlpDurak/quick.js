...
async function antiMassEmoji(message, options = {}) {
    ...
    try {
        ...
        const defaultEmoji = message.content.match(re.default) // returns array of matches with default emojis
        const customEmoji = message.content.match(re.custom) // returns array of matches with custom emojis

        let massEmojiUsage;

        //////////////////////////////
        /// MASS EMOJI USAGE CHECK ///
        //////////////////////////////
        if (defaultEmoji !== null || customEmoji !== null) {
            ...
            if (defaultEmoji !== null || defaultEmoji.length >= emojiLimit) massEmojiUsage = true; // checks for default emoji spam
            if (customEmoji !== null || customEmoji.length >= emojiLimit) massEmojiUsage = true; // checks for custom emoji spam
        }
